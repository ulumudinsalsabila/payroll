...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            const baseUrl = `{{ url('payroll-periods') }}`;
            const dataUrl = `{{ route('payroll-periods.data') }}`;
            ...
            const dt = $('#periods_table').DataTable({
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        name: 'month',
                        render: function(data) {
                            return esc(monthNames[data] || data);
                        }
                    },
                    {
                        data: 'description',
                    ...
                    },
                    {
                        ...
                        render: function(data) {
                            ...
                            const cls = isDraft ? 'warning' : 'success';
                            return '<span class="badge bg-light-' + cls + ' text-' + cls + '">' +
                                esc(data) + '</span>';
                        }
                    },
                    {
                        ...
                        render: function(row) {
                            const id = row.id;
                            return '' +
                                '<a href="' + baseUrl + '/' + esc(id) +
                                '" class="btn btn-light btn-sm me-2"><i class="bi bi-eye"></i> Detail</a>' +
            ...
            $('#periods_table').on('click', '.btnDeletePeriod', function() {
                ...
                const rowData = dt.row(tr).data() || {};
                const id = rowData.id;
                const mm = rowData.month;
                const label = (monthNames[mm] || mm) + ' ' + (rowData.year || '-');
