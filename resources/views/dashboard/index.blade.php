                        ...
                        <div>
                            ...
                            <div class="fs-5 fw-bold">
                                @if ($latestPeriod)
                                    @php
                                        try {
                                            $periodLabel = \Carbon\Carbon::createFromDate(
                                                (int) $latestPeriod->year,
                                                (int) $latestPeriod->month,
                                                1,
                                            )->translatedFormat('F Y');
                                        } catch (\Throwable $e) {
                                            $periodLabel =
                                                str_pad((string) $latestPeriod->month, 2, '0', STR_PAD_LEFT) .
                                                '/' .
                                                (string) $latestPeriod->year;
                                        }
                                    @endphp
                                    {{ $periodLabel }}
                                @else
                            ...
                            <div class="mt-2">
                                @if ($latestPeriod)
                                    ...
                                    <span
                                        class="badge {{ $status === 'published' ? 'badge-light-success' : 'badge-light-warning' }}">
                                        {{ strtoupper($status) }}
                                    </span>
                                @else
